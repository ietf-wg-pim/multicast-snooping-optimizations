...
TODO: Is there a disrepancy between requiring General Query messages to be forwarded to multicast router ports, while at the same time being able to use the absense of a Query with a non-zero source address to mean that there is no multicast router?
